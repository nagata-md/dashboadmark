...
export function buildSnapshot(dataset: ClientDataset, period: ResolvedReportPeriod): ReportPeriodSnapshot {
  ...
  const channelBreakdown = buildChannelBreakdown(dataset.campaignRows, dataset.channels, period.range, dataset.campaignTargets);
  return {
    ...
    targetVsActual: appendBudgetRow(
      buildTargetVsActual(funnel, dataset.targets, dataset.campaignTargets, period.range),
      channelBreakdown,
