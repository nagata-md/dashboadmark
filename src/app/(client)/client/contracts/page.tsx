...
import { saveContracts } from "./actions";

export const metadata = {
  title: "契約入力 | 住宅マーケティング数値ダッシュボード（仮称）",
};

type SearchParams = PeriodParams & { locationId?: string; success?: string };

// spec §4.3：契約（成約）数の入力（住宅会社側）。契約金額（売上）はv1では扱わない。
export default async function ContractsPage({
  ...
  return (
    <>
      ...
      {period ? (
        <Panel title="契約（成約）数" className="max-w-[420px]">
          <form action={saveContracts}>
            ...
            <FormRow label="契約（成約）数">
              <input
                ...
                name="contracts"
                min={0}
                defaultValue={existing?.contracts ?? 0}
              />
            </FormRow>
            <Button type="submit" variant="primary">
              保存
