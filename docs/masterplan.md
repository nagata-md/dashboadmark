...
10. **§1-1：施策データ入力を「一覧行→モーダル編集」方式に移植**（improvement.md §1-1、spec自身が「最重要課題」と位置づけていた項目）：
    ...
    - 検証：`npx tsc --noEmit`（`.next`キャッシュに残っていた削除済み`entry`ページへの型参照は`rm -rf .next`で解消）・`npm run lint`・`npm run test`（33件）・`npm run build`すべて成功（`/agency/clients/[id]/campaigns/entry`がルート一覧から消えたことを確認）。**この変更（ステップ10）はまだコミットしていない。**

**このセッションで完了していないこと（次回に持ち越し）**：

- ステップ10.（§1-1対応）の`git commit`・push。
- ステップ10.のブラウザ実機確認（施策データ入力の一覧行→モーダル編集、`/campaigns/entry`への直接アクセスが404になることも含む）。
...
1. `git status -sb`でステップ10.の変更が未コミットのまま残っているか確認する。
2. コミット・push可否をユーザーに確認する。
3. ステップ10.のブラウザ実機確認をユーザーに依頼する。
4. 確認が済み次第、§5-1（PDF印刷CSS改善）に着手する。

**次にやること（優先順、承認済み）**：①ステップ10.変更のコミット・push可否の確認、②ユーザーによる§1-1の実機確認、③§5-1（PDF印刷CSS改善）、④§1-2（来場〜契約入力の画面統合、着手前に設計方針をユーザー確認）。
