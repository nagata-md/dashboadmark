...
- §9-4（外部連携ID）以外の他のバックログ項目（§1・§2・§5・§6等）は未着手のまま。

**次回セッション開始時に最初に確認すること**：

1. `git log -1`で`c865a9d`が残っているか確認する（`git status -sb`は clean のはず）。
2. `docs/improvement.md`の優先度サマリー（§7）を見て、次に着手する項目をユーザーと確認する。

**次にやること（優先順、ユーザー未確認）**：バックログ（improvement.md §7優先度サマリー）に残る未着手項目のうち、ユーザーが次に着手したいものを確認する。
