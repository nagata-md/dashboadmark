...
- **次に着手する項目は§1-1（施策データ入力のモーダル化）とユーザー承認済み**（2026-08-10のモックアップ`CampaignsView.tsx`のモーダル編集パターンを実データ版に移植する）。その後§5-1（PDF、まず印刷CSSのみ）→§1-2（来場〜契約入力の画面統合、設計判断が必要）と続く予定。

**次回セッション開始時に最初に確認すること**：

1. `git log -1`で`71c4039`が残っているか確認する（`git status -sb`は clean のはず）。
2. §1-1（施策データ入力のモーダル化）に着手する。2026-08-10のモックアップ（`CampaignsView.tsx`）のモーダル編集パターンを`/agency/clients/[id]/campaigns`の実データ版に移植する。

**次にやること（優先順、承認済み）**：①§1-1（施策データ入力のモーダル化）、②§5-1（PDF印刷CSS改善）、③§1-2（来場〜契約入力の画面統合、着手前に設計方針をユーザー確認）。
