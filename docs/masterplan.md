...
12. **期間未選択時のデフォルト表示（月次・今月）を追加**（ユーザー指摘：「毎回期間を表示しないとデータが表示されないが、今日の日付をもとにデフォルトで月次・今月が表示されるようにしたい」）：
    ...
    - 検証：`npx tsc --noEmit`・`npm run lint`・`npm run test`（33件）・`npm run build`すべて成功。devサーバーは編集中も稼働中で、ホットリロード後も200応答を確認。**この変更（ステップ12）はまだコミットしていない。**

**このセッションで完了していないこと（次回に持ち越し）**：

- ステップ12.（デフォルト期間表示）の`git commit`・push。
- ステップ12.のブラウザ実機確認（施策データ・来場・見積/図面出し・契約の4画面で、クエリ無しの初回アクセス時に月次・今月のデータが自動表示されることを確認）。
...
- **次に着手する項目は§5-1（PDF、まず印刷CSSのみ）とユーザー承認済み**。その後§1-2（来場〜契約入力の画面統合、設計判断が必要）と続く予定。

**次回セッション開始時に最初に確認すること**：

1. `git status -sb`でステップ12.の変更が未コミットのまま残っているか確認する。
2. コミット・push可否をユーザーに確認する。
3. ステップ12.のブラウザ実機確認をユーザーに依頼する。
4. 確認が済み次第、§5-1（PDF印刷CSS改善）に着手する。

**次にやること（優先順、承認済み）**：①ステップ12.変更のコミット・push可否の確認、②ユーザーによる動作確認、③§5-1（PDF印刷CSS改善）、④§1-2（来場〜契約入力の画面統合、着手前に設計方針をユーザー確認）。
